...
function getCustomValidatorLessThan(fieldNameSmaller: string): CustomValidator {
  return (input, { req }) => {
    ...
    try {
      ...
      throw new Error(
        `Field ${fieldNameSmaller} cannot be converted to integer`,
      );
    }

    if (maxVal < minVal) {
      throw new Error(
        `Attribute ${fieldNameSmaller} (${minVal}) may not exceed maximum value`,
      );
    }
...
export function getValidationChains(): Array<ValidationChain> {
  let validationChains: Array<ValidationChain> = [
    ...
    body("packageDepth")
      .exists()
      .isInt({ min: 0, max: constants.MAX_PACKAGE_DEPTH }),
    body("minClassCount")
      .exists()
      .isInt({ min: 1, max: constants.MAX_CLASS_COUNT }),
    body("maxClassCount")
      .exists()
      .isInt({ min: 1, max: constants.MAX_CLASS_COUNT }),
    body("minMethodCount")
      .exists()
      .isInt({ min: 1, max: constants.MAX_METHODS }),
    body("maxMethodCount")
      .exists()
      .isInt({ min: 1, max: constants.MAX_METHODS }),
    body("duration")
      .exists()
      .isInt({ min: 1, max: constants.MAX_TRACE_DURATION }),
    body("callCount").exists().isInt({ min: 1, max: constants.MAX_CALL_COUNT }),
    body("maxCallDepth")
      .exists()
      .isInt({ min: 0, max: constants.MAX_CALL_DEPTH }),
    ...
    body("maxClassCount")
      .exists()
      .custom(getCustomValidatorLessThan("minClassCount")),
    body("maxMethodCount")
      .exists()
      .custom(getCustomValidatorLessThan("minMethodCount")),
  ];
